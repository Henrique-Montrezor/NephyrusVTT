/**
 * TableEngine — renderização da Mesa 2D com PixiJS (View no MVC).
 *
...
import { CONDITION_DEFS, BADGE_ICONS } from "@/lib/token-icons";
import type { Token, GridState, TokenLayer } from "@/net/types";

/** Mapa de condição por chave (cor + ícone). */
const CONDITION_MAP: Record<string, { key: string; label: string; color: string; svg: string }> =
  Object.fromEntries(CONDITION_DEFS.map((c) => [c.key, c]));

/** Ponto simples em coordenadas de tela ou mundo. */
interface Vec2 {
  x: number;
  y: number;
}

/** Névoa de guerra: retângulo escuro + células reveladas (Set "cx,cy"). */
interface FogState {
  enabled?: boolean;
  cells?: Set<string>;
}

/** Fonte de luz que abre um buraco na névoa (px). */
...
/** Callbacks de ferramenta ativa (coordenadas do mundo). */
interface ToolCallbacks {
  onDown?: (worldPoint: Point, event: FederatedPointerEvent) => void;
  onMove?: (worldPoint: Point, event: FederatedPointerEvent) => void;
  onUp?: () => void;
}

/** Traço de caneta persistente. */
...
/** Métrica persistente (círculo/cone/linha/régua). */
...
/** View de um token: Container com metadados de tamanho/id. */
interface TokenView extends Container {
  _tokenId?: number;
  _w: number;
  _h: number;
}
...
/** Callbacks públicos (todos opcionais) fornecidos ao construtor. */
export interface TableEngineCallbacks {
  onTokenDragEnd?: (id: number, x: number, y: number) => void;
  canControlToken?: (token: Token) => boolean;
  onTokenContextMenu?: (id: number, clientX: number, clientY: number) => void;
  onTokenResizeEnd?: (id: number, w: number, h: number) => void;
  onFogPaint?: (cells: [number, number][], reveal: boolean) => void;
}
...
export class TableEngine {
  ...
  app!: Application;
  world!: Container; // container pan/zoom
  layers: Record<string, Container> = {};
  tokenViews: Map<number, TokenView> = new Map();
  grid: GridState = { enabled: true, size_px: 64, meters_per_square: 1.5 };
  ...
  fogEditMode: string | null = null; // null | "reveal" | "hide"
  private _fogStroke: Set<string> | null = null; // Set de células já pintadas no traço atual
  ...
  tool: string | null = null; // nome da ferramenta ativa (ou null)
  private _toolCb: ToolCallbacks | null = null; // { onDown, onMove, onUp } em coordenadas do mundo
  private _toolDragging = false;

  // Métricas persistentes (movíveis/apagáveis) e callbacks de rede.
  private _templateDrag: TemplateDragState | null = null;
  onTemplateMove: (id: number, x: number, y: number, x2: number, y2: number) => void = () => {};
  ...
  private _strokeViews: Map<number, Graphics> = new Map();
  private _textViews: Map<number, Text> = new Map();
  private _templateViews: Map<number, TemplateEntry> = new Map();

  // Recursos de textura e resolução de token.
  private _boardBg: Graphics | null = null;
  private _fogSprite: Sprite | null = null;
  private _fogRT: RenderTexture | null = null;
  private _lightGradTex: Texture | null = null;
  private _iconTex: Record<string, Texture | null> = {};
  private _lookupToken: ((id: number) => Token | undefined) | null = null;
  private _pulseT = 0;

  /**
   * @param mountEl Container onde o canvas é inserido.
   * @param callbacks Conjunto de callbacks públicos (todos opcionais).
   */
  ...
  clampPoint(x: number, y: number): Vec2 {
  ...
  private _clampTokenPos(x: number, y: number, w: number, h: number): Vec2 {
    const { width, height } = this._sceneSize;
    if (!width || !height) return { x, y };
    return {
  ...
  async setBackground(url: string | null, width: number, height: number): Promise<void> {
  ...
  private _paintBoard(board: Graphics, width: number, height: number): void {
  ...
  drawGrid(grid: GridState, width: number, height: number): void {
    this.grid = { ...grid };
   ...
   */
  drawFog(fog: FogState | null | undefined, width: number, height: number, isGm: boolean): void {
    this._sceneSize = { width, height };
  ...
  setLights(list: LightSource[] | null | undefined): void {
    this._lights = list || [];
  }

  // --- Ferramentas / anotações (desenho, texto, régua, raio, magia) ---

  private _worldPoint(e: FederatedPointerEvent): Point {
    return this.world.toLocal(e.global);
  }

  /** Define a ferramenta ativa e seus callbacks (mundo). null = seleção. */
  setTool(name: string | null, callbacks: ToolCallbacks | null): void {
    this.tool = name || null;
  ...
  setTemplateCallbacks({ onMove, onDelete }: {
    onMove?: (id: number, x: number, y: number, x2: number, y2: number) => void;
  ...
  addTemplate(t: TemplateData): void {
    ...
    container.on("rightdown", (e: FederatedPointerEvent) => {
      e.stopPropagation();
  ...
  moveTemplate(id: number, x: number, y: number, x2: number, y2: number): void {
  ...
  private _pulseTemplates(tk: Ticker): void {
  ...
  private _startTemplateDrag(id: number, e: FederatedPointerEvent): void {
  ...
  private _onTemplateDragMove(e: FederatedPointerEvent): void {
  ...
  showMetricPreview(shape: string, a: Vec2, b: Vec2, color: string | number, label?: string): void {
  ...
  private _measureLabel(text: string, x: number, y: number): void {
    const label = new Text({
      text,
      style: {
        fill: 0xffffff,
  ...
  showStrokePreview(points: number[] | null | undefined, color: string | number): void {
    const layer = this.layers.measure;
    layer.removeChildren();
  ...
  private _cellFromEvent(event: FederatedPointerEvent): { cx: number; cy: number } {
    const p = this.world.toLocal(event.global);
    const step = this.grid.size_px || 64;
    return { cx: Math.floor(p.x / step), cy: Math.floor(p.y / step) };
  }

  private _startFogStroke(event: FederatedPointerEvent): void {
    this._fogStroke = new Set();
    this._paintFogCell(event);
    event.stopPropagation();
  }

  private _onFogPaintMove(event: FederatedPointerEvent): void {
    if (!this._fogStroke) return;
    this._paintFogCell(event);
  }

  private _paintFogCell(event: FederatedPointerEvent): void {
  ...
  private _tokenSize(token: Token): { w: number; h: number } {
  ...
  private _createTokenView(token: Token): TokenView {
    const view = new Container() as TokenView;
    ...
    view.on("pointerdown", (e: FederatedPointerEvent) => this._startDrag(token.id, e));
    view.on("rightdown", (e: FederatedPointerEvent) => {
      e.stopPropagation();
      const oe = (e.nativeEvent ?? e) as MouseEvent;
      this.onTokenContextMenu(token.id, oe.clientX ?? 0, oe.clientY ?? 0);
    });

    if (token.imageUrl) {
      Assets.load<Texture>(token.imageUrl)
        .then((tex) => {
          img.texture = tex;
  ...
  private _applyTokenView(view: TokenView, token: Token): void {
    const { w, h } = this._tokenSize(token);
    view.x = token.x;
    view.y = token.y;
    view.alpha = token.isHidden ? 0.55 : 1;

    const img = view.getChildByLabel("img") as Sprite;
    const ph = view.getChildByLabel("ph") as Graphics;
    ph.clear();
    ...
    const label = view.getChildByLabel("label") as Text;
    ...
    view._w = w;
    view._h = h;
    this._drawTokenBadges(view, token, w);
  }

  /** Desenha os ícones de condição/travado/luz sobre o token. */
  private _drawTokenBadges(view: TokenView, token: Token, w: number, _h?: number): void {
    const badges = view.getChildByLabel("badges") as Container;
  ...
  deselectToken(): void {
    ...
    (view?.getChildByLabel("handles") as Container | null)?.removeChildren();
    this.selectedId = null;
  }

  private _drawHandles(view: TokenView): void {
    const handles = view.getChildByLabel("handles") as Container;
    ...
    for (const [corner, hx, hy] of corners) {
      ...
      g.on("pointerdown", (e: FederatedPointerEvent) => this._startResize(view._tokenId!, corner, e));
      handles.addChild(g);
    }
  }

  private _startResize(tokenId: number, corner: string, event: FederatedPointerEvent): void {
  ...
  private _onResizeMove(event: FederatedPointerEvent): void {
  ...
  private _applyLiveSize(view: TokenView, w: number, h: number): void {
    const img = view.getChildByLabel("img") as Sprite;
    const ph = view.getChildByLabel("ph") as Graphics;
    if (img.visible) {
      img.width = w;
  ...
  private _startDrag(tokenId: number, event: FederatedPointerEvent): void {
  ...
  setTokenResolver(fn: (id: number) => Token | undefined): void {
...
function _readTableBg(): string {
  return _readVar("--table-bg", "#c9d0e2");
}

/** Converte uma cor CSS "#rrggbb" em número (0xRRGGBB). */
function _toHex(css: string | number | undefined, fallback: number): number {
...
function _makeLightTexture(): Texture {
  const size = 256;
...
function _svgTexture(inner: string, color = "#ffffff"): Promise<Texture | null> {
