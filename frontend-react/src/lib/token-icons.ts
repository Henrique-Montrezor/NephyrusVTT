/**
 * Ícones SVG (sem emojis) para menus e badges na mesa. viewBox 0 0 24 24.
 * Ações usam currentColor; condições/badges usam currentColor sobre círculo
 * colorido no token. Portado de token_icons.js.
 */

/** Conteúdo interno (inner SVG) dos ícones de ação do menu. */
export const ICONS = {
  rename:
...
/** Monta o markup de um <svg> a partir do conteúdo interno. */
export function svgMarkup(inner: string, size = 18): string {
...
/** Condições: cor do badge + ícone (currentColor). */
...
/** Ícones dos indicadores de travado/luz no token. */
